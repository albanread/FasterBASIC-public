...
pub fn build(b: *std.Build) !void {
    ...
    if (!skip_install_lib_files) {
        ...
        b.installDirectory(.{
            .source_dir = b.path("macgui"),
            .install_dir = if (flat) .prefix else .lib,
            .install_subdir = if (flat) "lib/macgui" else "zig/macgui",
            .exclude_extensions = &[_][]const u8{
                ".DS_Store",
            },
        });
    ...
    const jit_shared_mod = b.createModule(.{
        .root_source_file = b.path("lib/compiler/basic/jit_shared.zig"),
        .target = b.graph.host,
        .optimize = optimize,
    });

    const edglue_exe = b.addExecutable(.{
        .name = "edglue",
        .root_module = b.createModule(.{
            .root_source_file = b.path("tools/edglue.zig"),
            .target = b.graph.host,
            .optimize = optimize,
        }),
    });
    edglue_exe.root_module.addImport("jit_shared", jit_shared_mod);
    const install_edglue = b.addInstallArtifact(edglue_exe, .{
        .dest_dir = if (flat) .{ .override = .prefix } else .default,
    });
    const edglue_step = b.step("edglue", "Build the Edglue shared-memory bridge executable");
    edglue_step.dependOn(&install_edglue.step);

    const testedglue_exe = b.addExecutable(.{
        .name = "testedglue",
        .root_module = b.createModule(.{
            .root_source_file = b.path("tools/testedglue.zig"),
            .target = b.graph.host,
            .optimize = optimize,
        }),
    });
    testedglue_exe.root_module.addImport("jit_shared", jit_shared_mod);
    const install_testedglue = b.addInstallArtifact(testedglue_exe, .{
        .dest_dir = if (flat) .{ .override = .prefix } else .default,
    });
    const testedglue_step = b.step("testedglue", "Build the TestEdglue protocol tester executable");
    testedglue_step.dependOn(&install_testedglue.step);
...
fn addCompilerMod(b: *std.Build, options: AddCompilerModOptions) *std.Build.Module {
    ...
    const basic_compiler_mod = b.createModule(.{
        .root_source_file = b.path("lib/compiler/basic/compiler_llvm.zig"),
    });
    basic_compiler_mod.addSystemIncludePath(.{ .cwd_relative = "deps/llvm20-install/include" });
    ...
    compiler_mod.addImport("basic_compiler", basic_compiler_mod);
