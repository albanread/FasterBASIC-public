...
const basic_runtime = @import("libs/basic_runtime.zig");
const Zir = std.zig.Zir;
const Air = @import("Air.zig");
const Builtin = @import("Builtin.zig");
const LlvmObject = @import("codegen/llvm.zig").Object;
const dev = @import("dev.zig");
const basic_compiler = @import("basic_compiler");
...
basic_object_table: std.AutoArrayHashMapUnmanaged(*BasicObject, void) = .empty,
...
/// These jobs are to invoke the integrated BASIC compiler to create an object file,
/// which gets linked with the Compilation.
basic_object_work_queue: DeprecatedLinearFifo(*BasicObject),
...
/// The ErrorBundle memory is owned by the `BasicObject`, using Compilation's general purpose allocator.
/// This data is accessed by multiple threads and is protected by `mutex`.
failed_basic_objects: std.AutoArrayHashMapUnmanaged(*BasicObject, ErrorBundle) = .empty,
...
basic_source_files: []const BasicSourceFile,
...
/// Populated when we build the BASIC runtime static library. A Job to build this is
/// placed in the queue and resolved before calling linker.flush().
basic_runtime_lib: ?CrtFile = null,
/// Populated when a shipped BASIC runtime archive is present on disk and used instead
/// of building the runtime late inside the compiler.
basic_runtime_prebuilt_path: ?Cache.Path = null,
...
/// For passing to the integrated BASIC compiler pipeline.
pub const BasicSourceFile = struct {
    owner: *Package.Module,
    src_path: []const u8,
    extra_flags: []const []const u8 = &.{},
};
...
/// A compiled BASIC source file: parallel to CObject but uses the
/// integrated BASIC pipeline instead of clang.
pub const BasicObject = struct {
    /// Relative to cwd. Owned by arena.
    src: BasicSourceFile,
    status: union(enum) {
        new,
        success: struct {
            /// The outputted result. `sub_path` owned by gpa.
            object_path: Cache.Path,
            /// File system lock on the cache hash manifest.
            lock: Cache.Lock,
        },
        /// There will be a corresponding ErrorBundle in Compilation.failed_basic_objects.
        failure,
        /// A transient failure happened; it may succeed if we try again.
        failure_retryable,
    },

    /// Returns true if there was failure.
    pub fn clearStatus(self: *BasicObject, gpa: Allocator) bool {
        switch (self.status) {
            .new => return false,
            .failure, .failure_retryable => {
                self.status = .new;
    ...
    pub fn destroy(self: *BasicObject, gpa: Allocator) void {
        _ = self.clearStatus(gpa);
        gpa.destroy(self);
    }
};
...
pub const cache_helpers = struct {
    ...
    pub fn hashBasicSource(self: *Cache.Manifest, basic_source: BasicSourceFile) !void {
        _ = try self.addFile(basic_source.src_path, null);
        // BASIC objects are produced by the integrated BASIC compiler, so the
        // cache key must move when that compiler changes as well.
        self.hash.addBytes(build_options.version);
        self.hash.addBytes("basic-object-cache-v2");
        for (basic_source.extra_flags) |arg| {
            self.hash.addBytes(arg);
        }
    }
};
...
pub fn create(gpa: Allocator, arena: Allocator, diag: *CreateDiagnostic, options: CreateOptions) error{
    ...
    // Add a `BasicObject` for each `basic_source_files`.
    try comp.basic_object_table.ensureTotalCapacity(gpa, options.basic_source_files.len);
    for (options.basic_source_files) |basic_source_file| {
        const basic_object = try gpa.create(BasicObject);
        errdefer gpa.destroy(basic_object);

        basic_object.* = .{
            .status = .{ .new = {} },
            .src = basic_source_file,
        };
        comp.basic_object_table.putAssumeCapacityNoClobber(basic_object, {});
    }
    ...
    if (comp.emit_bin != null and target.ofmt != .c) {
        if (!comp.skip_linker_dependencies) {
            ...
            if (is_exe_or_dyn_lib and comp.basic_object_table.count() != 0) {
                comp.queued_jobs.basic_runtime = true;
            }
...
pub fn destroy(comp: *Compilation) void {
    ...
    comp.basic_object_work_queue.deinit();
    ...
    if (comp.basic_runtime_lib) |*crt_file| {
        crt_file.deinit(gpa);
    }
    if (comp.basic_runtime_prebuilt_path) |*path| {
        gpa.free(path.sub_path);
    }
    ...
    for (comp.basic_object_table.keys()) |key| {
        key.destroy(gpa);
    }
    comp.basic_object_table.deinit(gpa);

    for (comp.failed_basic_objects.values()) |*value| {
        value.deinit(gpa);
    }
    comp.failed_basic_objects.deinit(gpa);
...
pub fn update(comp: *Compilation, main_progress_node: std.Progress.Node) UpdateError!void {
    ...
    // For compiling BASIC objects, we rely on the cache hash system to avoid duplicating work.
    // Add a Job for each BASIC source file.
    try comp.basic_object_work_queue.ensureUnusedCapacity(comp.basic_object_table.count());
    for (comp.basic_object_table.keys()) |basic_object| {
        comp.basic_object_work_queue.writeItemAssumeCapacity(basic_object);
        try comp.appendFileSystemInput(try .fromUnresolved(arena, comp.dirs, &.{basic_object.src.src_path}));
    }
...
pub fn getAllErrorsAlloc(comp: *Compilation) error{OutOfMemory}!ErrorBundle {
    ...
    for (comp.failed_basic_objects.values()) |error_bundle| {
...
fn performAllTheWork(
    ...
    if (comp.basic_object_table.count() != 0 and comp.getBasicRuntimePath() == null) {
        comp.link_task_queue.startPrelinkItem();
        comp.link_task_wait_group.spawnManager(buildBasicRuntime, .{ comp, main_progress_node });
    }
...

    {
        ...
        while (comp.basic_object_work_queue.readItem()) |basic_object| {
            comp.link_task_queue.startPrelinkItem();
            comp.thread_pool.spawnWg(&comp.link_task_wait_group, workerUpdateBasicObject, .{
                comp, basic_object, main_progress_node,
            });
        }
...
fn buildBasicRuntime(comp: *Compilation, prog_node: std.Progress.Node) void {
    defer comp.link_task_queue.finishPrelinkItem(comp);
    if (basic_runtime.buildStaticLib(comp, prog_node)) |_| {
        comp.queued_jobs.basic_runtime = false;
    } else |err| switch (err) {
        error.AlreadyReported => return,
        else => comp.lockAndSetMiscFailure(.basic_runtime, "unable to build BASIC runtime library: {s}", .{@errorName(err)}),
    }
}
...
fn workerUpdateBasicObject(
    comp: *Compilation,
    basic_object: *BasicObject,
    progress_node: std.Progress.Node,
) void {
    defer comp.link_task_queue.finishPrelinkItem(comp);
    comp.updateBasicObject(basic_object, progress_node) catch |err| switch (err) {
        error.AnalysisFail => return,
        else => {
            comp.reportRetryableBasicObjectError(basic_object, err) catch |oom| switch (oom) {
                error.OutOfMemory => {},
            };
        },
    };
...
fn updateBasicObject(comp: *Compilation, basic_object: *BasicObject, prog_node: std.Progress.Node) anyerror!void {
    if (!build_options.have_llvm) {
        return comp.failBasicObject(basic_object, "BASIC compilation requires LLVM (compiler built without LLVM extensions)", .{});
    }

    const tracy_trace = trace(@src());
    defer tracy_trace.end();

    log.debug("updating BASIC object: {s}", .{basic_object.src.src_path});

    const gpa = comp.gpa;

    if (basic_object.clearStatus(gpa)) {
        comp.mutex.lock();
        defer comp.mutex.unlock();
        _ = comp.failed_basic_objects.swapRemove(basic_object);
    }

    var man = comp.obtainCObjectCacheManifest(basic_object.src.owner);
    defer man.deinit();

    try cache_helpers.hashBasicSource(&man, basic_object.src);

    var arena_allocator = std.heap.ArenaAllocator.init(gpa);
    defer arena_allocator.deinit();
    const arena = arena_allocator.allocator();

    const src_basename = fs.path.basename(basic_object.src.src_path);
    const child_progress_node = prog_node.start(src_basename, 0);
    defer child_progress_node.end();

    const o_basename_noext = src_basename[0 .. src_basename.len - fs.path.extension(src_basename).len];
    const target = comp.getTarget();
    const o_ext = target.ofmt.fileExt(target.cpu.arch);
    const o_basename = try std.fmt.allocPrint(arena, "{s}{s}", .{ o_basename_noext, o_ext });

    var basic_debug_opts: basic_compiler.CompileDebugOptions = .{};
    for (basic_object.src.extra_flags) |flag| {
        if (mem.eql(u8, flag, "--trace-ast")) {
            basic_debug_opts.trace_ast = true;
        } else if (mem.eql(u8, flag, "--trace-cfg")) {
            basic_debug_opts.trace_cfg = true;
        } else if (mem.eql(u8, flag, "--trace-symbols")) {
            basic_debug_opts.trace_symbols = true;
        } else if (mem.eql(u8, flag, "--show-tokens")) {
            basic_debug_opts.show_tokens = true;
        }
    }
    ...
    const digest = if (try man.hit()) man.final() else blk: {
        ...
        const source = try fs.cwd().readFileAlloc(arena, basic_object.src.src_path, std.math.maxInt(usize));
        const llvm_triple = try @import("codegen/llvm.zig").targetTriple(arena, target);

        const opt_level: u8 = switch (basic_object.src.owner.optimize_mode) {
            .Debug => 0,
            .ReleaseSafe => 1,
            .ReleaseFast, .ReleaseSmall => 2,
        };

        const owner_root_src = basic_object.src.owner.root_src_path;
        const owner_root_is_bas = std.mem.endsWith(u8, owner_root_src, ".bas");
        const src_base = fs.path.basename(basic_object.src.src_path);
        const owner_root_base = fs.path.basename(owner_root_src);
        const emit_entrypoint = owner_root_is_bas and std.mem.eql(u8, src_base, owner_root_base);

        var basic_result = try basic_compiler.compileToObject(
            arena,
            source,
            out_obj_path,
            llvm_triple,
        ...
        defer basic_result.deinit();

        if (!basic_result.success or basic_result.errors.items.len != 0) {
            var eb_wip: ErrorBundle.Wip = undefined;
            try eb_wip.init(gpa);
            errdefer eb_wip.deinit();
            for (basic_result.errors.items) |diag| {
                try eb_wip.addRootErrorMessage(.{
                    .msg = try eb_wip.printString("{s}", .{diag.message}),
                    .src_loc = try eb_wip.addSourceLocation(.{
                        .src_path = try eb_wip.addString(basic_object.src.src_path),
                        .line = diag.line -| 1,
                        .column = diag.col -| 1,
                        .span_start = 0,
                        .span_main = 0,
            ...
            if (basic_result.errors.items.len == 0) {
                try eb_wip.addRootErrorMessage(.{
                    .msg = try eb_wip.printString("BASIC compiler failed before object emission", .{}),
                    .src_loc = try eb_wip.addSourceLocation(.{
                        .src_path = try eb_wip.addString(basic_object.src.src_path),
                        .line = 0,
                        .column = 0,
            ...
            const eb = try eb_wip.toOwnedBundle("");
            return comp.failBasicObjectWithOwnedBundle(basic_object, eb);
        }

        // Guard against silent BASIC pipeline failures. If no object file was
        // written, stop before link and return a source-level failure.
        if (fs.cwd().access(out_obj_path, .{})) |_| {} else |_| {
            return comp.failBasicObject(
                basic_object,
                "BASIC object emission produced no output file: '{s}'",
                .{out_obj_path},
            );
        }

        if (comp.emit_llvm_ir) |raw_ir_path| {
            var ir_result = basic_compiler.compileToIR(arena, source, false);
            defer ir_result.deinit();

            if (!ir_result.success or ir_result.text == null) {
                const msg = if (ir_result.errors.items.len > 0)
                    ir_result.errors.items[0].message
                else
                    "failed to emit LLVM IR for BASIC source";
                return comp.failBasicObject(basic_object, "{s}", .{msg});
            }

            const emit_path = comp.resolveEmitPath(raw_ir_path);
            if (fs.path.dirname(emit_path.sub_path)) |dir| {
                try emit_path.root_dir.handle.makePath(dir);
            }
            try emit_path.root_dir.handle.writeFile(.{
                .sub_path = emit_path.sub_path,
                .data = ir_result.text.?,
            });
        }

        if (comp.emit_asm) |raw_asm_path| {
            var asm_result = basic_compiler.compileToAsm(arena, source, false);
            defer asm_result.deinit();

            if (!asm_result.success or asm_result.text == null) {
                const msg = if (asm_result.errors.items.len > 0)
                    asm_result.errors.items[0].message
                else
                    "failed to emit assembly for BASIC source";
                return comp.failBasicObject(basic_object, "{s}", .{msg});
            }

            const emit_path = comp.resolveEmitPath(raw_asm_path);
            if (fs.path.dirname(emit_path.sub_path)) |dir| {
                try emit_path.root_dir.handle.makePath(dir);
            }
            try emit_path.root_dir.handle.writeFile(.{
                .sub_path = emit_path.sub_path,
                .data = asm_result.text.?,
            });
        }

        const digest_val = man.final();
        const o_sub_path = try fs.path.join(arena, &[_][]const u8{ "o", &digest_val });
        var o_dir = try comp.dirs.local_cache.handle.makeOpenPath(o_sub_path, .{});
        defer o_dir.close();
        const tmp_basename = fs.path.basename(out_obj_path);
        try fs.rename(zig_cache_tmp_dir, tmp_basename, o_dir, o_basename);
        break :blk digest_val;
    };

    if (man.have_exclusive_lock) {
        man.writeManifest() catch |err| {
            log.warn("failed to write cache manifest for '{s}': {s}", .{ basic_object.src.src_path, @errorName(err) });
        };
    }
    ...
    // If a stale/missing cache entry slips through, fail now instead of
    // forwarding an invalid object path to the linker.
    if (basic_object.status.success.object_path.root_dir.handle.access(basic_object.status.success.object_path.sub_path, .{})) |_| {} else |_| {
        return comp.failBasicObject(
            basic_object,
            "BASIC object cache entry is missing: '{f}'",
            .{basic_object.status.success.object_path},
        );
    }

    comp.queuePrelinkTasks(&.{.{ .load_object = basic_object.status.success.object_path }});
}

fn reportRetryableBasicObjectError(
    comp: *Compilation,
    basic_object: *BasicObject,
    err: anyerror,
) error{OutOfMemory}!void {
    basic_object.status = .failure_retryable;
    ...
    try bundle.addRootErrorMessage(.{
        .msg = try bundle.printString("{s}", .{@errorName(err)}),
        .src_loc = try bundle.addSourceLocation(.{
            .src_path = try bundle.addString(basic_object.src.src_path),
            .line = 0,
            .column = 0,
            .span_start = 0,
            .span_main = 0,
    ...
    const finished_bundle = try bundle.toOwnedBundle("");
    {
        comp.mutex.lock();
        defer comp.mutex.unlock();
        try comp.failed_basic_objects.putNoClobber(comp.gpa, basic_object, finished_bundle);
    }
}
...
fn failBasicObject(
    ...
    const finished_bundle = try bundle.toOwnedBundle("");
    return comp.failBasicObjectWithOwnedBundle(basic_object, finished_bundle);
}

fn failBasicObjectWithOwnedBundle(
    comp: *Compilation,
    basic_object: *BasicObject,
    err_bundle: ErrorBundle,
) SemaError {
    @branchHint(.cold);
    {
        comp.mutex.lock();
        defer comp.mutex.unlock();
        try comp.failed_basic_objects.putNoClobber(comp.gpa, basic_object, err_bundle);
    }
    basic_object.status = .failure;
    return error.AnalysisFail;
}
...
pub fn classifyFileExt(filename: []const u8) FileExt {
    if (hasCExt(filename)) {
        ...
        return .zig;
    } else if (std.ascii.endsWithIgnoreCase(filename, ".bas")) {
        return .basic;
...
pub fn getBasicRuntimePath(comp: *const Compilation) ?Cache.Path {
    if (comp.basic_runtime_lib) |lib| return lib.full_object_path;
    if (comp.basic_runtime_prebuilt_path) |path| return path;
    return null;
}
