...
fn linkAsArchive(lld: *Lld, arena: Allocator) !void {
    ...
    const compiler_rt_path: ?Cache.Path = if (comp.compiler_rt_strat == .obj)
        ...
        null;
    const basic_runtime_path: ?Cache.Path = comp.getBasicRuntimePath();
    ...
    try object_files.ensureUnusedCapacity(arena, comp.c_object_table.count() +
        comp.basic_object_table.count() + comp.win32_resource_table.count() + 3);
    ...
    if (basic_runtime_path) |p| object_files.appendAssumeCapacity(try p.toStringZ(arena));
...
fn coffLink(lld: *Lld, arena: Allocator) !void {
    ...
    if (comp.config.output_mode == .Obj) {
        ...
        const the_object_path = blk: {
            ...
            if (comp.basic_object_table.count() != 0)
                break :blk comp.basic_object_table.keys()[0].status.success.object_path;
        ...
        if (is_exe_or_dyn_lib and !comp.skip_linker_dependencies) {
            ...
            if (comp.getBasicRuntimePath()) |path| try argv.append(try path.toString(arena));
        }
