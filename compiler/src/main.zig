...
const normal_usage =
    ...
    \\  basic            Compile a FasterBASIC source file
    \\
...
fn mainArgs(gpa: Allocator, arena: Allocator, args: []const []const u8) !void {
    ...
    } else if (mem.eql(u8, cmd, "basic")) {
        dev.check(.basic_command);
        return buildOutputType(gpa, arena, args, .basic);
...
const CliModule = struct {
    ...
    basic_source_files_start: usize,
    basic_source_files_end: usize,
...
fn buildOutputType(
    ...
    var extra_basicflags: std.ArrayListUnmanaged([]const u8) = .empty;
    ...
    // Tracks the position in basic_source_files which have already their owner populated.
    var basic_source_files_owner_index: usize = 0;
    ...
    var create_module: CreateModule = .{
        ...
        .opts = .{
            .is_test = switch (arg_mode) {
                ...
                .build, .cc, .cpp, .basic, .translate_c, .run => false,
            },
    ...
    switch (arg_mode) {
        .build, .basic, .translate_c, .zig_test, .zig_test_obj, .run => {
            switch (arg_mode) {
                .build => |m| {
                ...
                if (mem.startsWith(u8, arg, "@")) {
                    ...
                    if (mem.eql(u8, arg, "-h") or mem.eql(u8, arg, "--help")) {
                        ...
                        try handleModArg(
                            ...
                            &basic_source_files_owner_index,
                            &cssan,
                        );
                    ...
                    } else if (arg_mode == .basic and mem.eql(u8, arg, "--trace-ast")) {
                        try extra_basicflags.append(arena, arg);
                    } else if (arg_mode == .basic and mem.eql(u8, arg, "--trace-cfg")) {
                        try extra_basicflags.append(arena, arg);
                    } else if (arg_mode == .basic and mem.eql(u8, arg, "--trace-symbols")) {
                        try extra_basicflags.append(arena, arg);
                    } else if (arg_mode == .basic and mem.eql(u8, arg, "--show-tokens")) {
                        try extra_basicflags.append(arena, arg);
                    } else {
                    ...
                    .basic => {
                        dev.check(.basic_command);
                        try create_module.basic_source_files.append(arena, .{
                            .owner = undefined,
                            .src_path = arg,
                            .extra_flags = try arena.dupe([]const u8, extra_basicflags.items),
                        });
                    },
                    .zig => {
        ...
        .cc, .cpp => {
            ...
            while (it.has_next) {
                ...
                switch (it.zig_equivalent) {
                    ...
                    .positional => switch (file_ext orelse Compilation.classifyFileExt(mem.sliceTo(it.only_arg, 0))) {
                        ...
                        .basic => {
                            fatal("BASIC source files are not supported by zig cc/c++; use 'zig basic'", .{});
                        },
            ...
            if (create_module.c_source_files.items.len == 0 and
                create_module.basic_source_files.items.len == 0 and
    ...
    implicit_root_mod: {
        const src_path = b: {
            ...
            if (create_module.basic_source_files.items.len >= 1)
                break :b create_module.basic_source_files.items[0].src_path;
        ...
        const name = switch (arg_mode) {
            .zig_test => "test",
            .build, .cc, .cpp, .basic, .translate_c, .zig_test_obj, .run => fs.path.stem(fs.path.basename(src_path)),
        };
        ...
        try create_module.modules.put(arena, name, .{
            ...
            .basic_source_files_start = basic_source_files_owner_index,
            .basic_source_files_end = create_module.basic_source_files.items.len,
        });
        cssan.reset();
        mod_opts = .{};
        ...
        basic_source_files_owner_index = create_module.basic_source_files.items.len;
    }
    ...
    if (create_module.basic_source_files.items.len != 0 and extra_basicflags.items.len != 0) {
        const basic_flags = try arena.dupe([]const u8, extra_basicflags.items);
        for (create_module.basic_source_files.items) |*item| {
            item.extra_flags = basic_flags;
        }
    }
    ...
    if (basic_source_files_owner_index != create_module.basic_source_files.items.len) {
        fatal("BASIC source file '{s}' has no parent module", .{
            create_module.basic_source_files.items[basic_source_files_owner_index].src_path,
        });
    }
    ...
    for (create_module.modules.keys(), create_module.modules.values()) |key, cli_mod| {
        ...
        create_module.opts.any_basic_source_files = create_module.basic_source_files.items.len != 0;
    }
    ...
    if (create_module.basic_source_files.items.len != 0) {
        function_sections = true;
        data_sections = true;
        if (linker_gc_sections == null) linker_gc_sections = true;
    }

    // BASIC runtime math uses vDSP/vv* symbols on Darwin.
    // macOS GUI/audio BASIC programs also need Cocoa/Metal/CoreAudio frameworks.
    if (create_module.basic_source_files.items.len != 0 and target.os.tag == .macos) {
        // Ensure framework resolution has a standard Darwin search path.
        if (create_module.framework_dirs.items.len == 0) {
            try create_module.framework_dirs.append(arena, "/System/Library/Frameworks");
            try create_module.framework_dirs.append(arena, "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks");
            try create_module.framework_dirs.append(arena, "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/System/Library/Frameworks");
        }
        const basic_frameworks = [_][]const u8{
            "Accelerate",
            "Cocoa",
            "Metal",
            "MetalKit",
        ...
        for (basic_frameworks) |framework_name| {
            if (create_module.frameworks.get(framework_name) == null) {
                try create_module.frameworks.put(arena, framework_name, .{});
            }
        }
    ...
    const comp = Compilation.create(gpa, arena, &create_diag, .{
        ...
        .basic_source_files = create_module.basic_source_files.items,
...
const CreateModule = struct {
    ...
    basic_source_files: std.ArrayListUnmanaged(Compilation.BasicSourceFile),
...
fn createModule(
    ...
    for (create_module.basic_source_files.items[cli_mod.basic_source_files_start..cli_mod.basic_source_files_end]) |*item| item.owner = mod;
...
fn handleModArg(
    ...
    basic_source_files_owner_index: *usize,
